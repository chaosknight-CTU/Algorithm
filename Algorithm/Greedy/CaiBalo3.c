...
void readList(List *L, int *weightOfBag){
	...
	while (!feof(f)){
        fscanf(f, "%d %d", &temp.weight, &temp.value);
...
void printList(List *L){
    int i;
    for (i=0; i < L->n; i++)
        printf("%d %d %s\n", L->items[i].weight, L->items[i].value, L->items[i].Name);
}
...
void bag(List *L, int weightOfBag){
    ...
    for (i=0; i < L->n; i++){
        ...
        /* neu so luong do vat co the lay lon hon 1 */
        if (L->items[i].solution > 1)
            L->items[i].solution = 1;
