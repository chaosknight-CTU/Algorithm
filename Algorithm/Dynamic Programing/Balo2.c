...
*/
typedef struct{
    int weight, value, maxQuantity, quantity;
    char name[MAX_N];
}Item;
...
void readList(List *pL){
    ...
    pL->n = 0;
    while (!feof(f)){
        fscanf(f, "%d %d %d", &pL->items[pL->n].weight, &pL->items[pL->n].value, &pL->items[pL->n].maxQuantity);
...
void printList(List *pL){
    int i;
    for (i=0; i < pL->n; i++)
        printf("%d %d %s %d\n", pL->items[i].weight, pL->items[i].value, pL->items[i].name, pL->items[i].quantity);
}
...
void initTable(List *pL, int X[][pL->w+1], int F[][pL->w+1]){
    ...
    for (V=0; V <= pL->w; V++){
        X[0][V] = min(V / pL->items[0].weight, pL->items[0].maxQuantity);
    ...
    for (k=1; k < pL->n; k++)
        for (V=0; V <= pL->w; V++){
            FMax = F[k-1][V];
            XMax = 0;
            yk = min(V / pL->items[k].weight, pL->items[k].maxQuantity);
